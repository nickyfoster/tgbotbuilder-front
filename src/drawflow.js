class Drawflow {
  ...
  addNodeOutputWithText(id){
    this.addNodeOutput(id);
    this.addNodeButton(id);
  }

  updateNodeButtonData(id, buttonId) {
    let nodeData = this.getNodeFromId(id).data;
    let newNodeData = {};
    newNodeData.text = nodeData.text;
    newNodeData.btn = {};

    if (nodeData.btn == undefined) {
      return
    }

    delete nodeData.btn[buttonId];

    let cnt = 1;
    if (Object.keys(nodeData.btn).length > 0) {
      for (const key in nodeData.btn) {
        newNodeData.btn[cnt] = nodeData.btn[key];
        cnt++;
      }
    }
    this.updateNodeDataFromId(id, newNodeData);
  }
  ...
  addNodeButton(id) {
    ...
    if(this.module === moduleName) {      
      //Draw button
      let buttonId = numOutputs;  // We just added new output. Let's use it as button ID.

      const button = document.createElement('div');
      const buttonTextInput = document.createElement("input");
      buttonTextInput.setAttribute(`df-btn-${buttonId}`, '');

      const deleteButton = document.createElement("button");
      deleteButton.textContent = "X";
      deleteButton.id = buttonId;
      deleteButton.onclick = () => {        
        this.removeNodeButton(id, deleteButton.id);
        this.removeNodeOutput(id, `output_${deleteButton.id}`);
        this.updateNodeButtonData(id, deleteButton.id);
      };

      button.appendChild(buttonTextInput);
      button.appendChild(deleteButton);
      button.classList.add("button");
      button.classList.add(`button-${buttonId}`);

      document.getElementById(`buttons_${id}`).appendChild(button);
    }
  }
  ...
  removeNodeButton(id, buttonId) {
    var moduleName = this.getModuleFromNodeId(id)
    const infoNode = this.getNodeFromId(id)
    if(this.module === moduleName) {
      this.container.querySelector('#node-'+id+' .drawflow_content_node .buttons .button.button-'+buttonId).remove();

      const eles = this.container.querySelectorAll('#node-'+id+' .drawflow_content_node .button');
      let cnt = 1;
      eles.forEach((item, i) => {
        let currId = item.classList[1].slice(7);
        item.classList.remove(item.classList[1]);
        item.classList.add(`button-${cnt}`);
        
        let input = item.querySelector('input');
        input.removeAttribute(`df-btn-${currId}`);
        input.setAttribute(`df-btn-${cnt}`, "");

        let button = item.querySelector('button');
        button.removeAttribute(`id-${currId}`);
